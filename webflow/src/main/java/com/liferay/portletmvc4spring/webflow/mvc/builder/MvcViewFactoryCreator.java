 ...
 *
 * <p>This implementation detects whether it is running in a Servlet or Portlet MVC environment, and returns instances
 * of the default view factory implementation for that environment.
 *
 * <p>By default, this implementation creates view factories that resolve their views by loading flow-relative
 * resources, such as .jsp templates located in a flow working directory. This class also supports rendering views
 * resolved by pre-existing Spring MVC {@link ViewResolver view resolvers}.
 *
...
public class MvcViewFactoryCreator implements ViewFactoryCreator, ApplicationContextAware {
	 ...
	 *
	 * <p>Default is "_", for "_FIELD" parameters (e.g. "_subscribeToNewsletter"). Set this to null if you want to turn
	 * off the empty field check completely.
	 *
	 * <p>HTML checkboxes only send a value when they're checked, so it is not possible to detect that a formerly
	 * checked box has just been unchecked, at least not with standard HTML means.
	 *
	 * <p>This auto-reset mechanism addresses this deficiency, provided that a marker parameter is sent for each
	 * checkbox field, like "_subscribeToNewsletter" for a "subscribeToNewsletter" field. As the marker parameter is
	 * sent in any case, the data binder can detect an empty field and automatically reset its value.
	 */
