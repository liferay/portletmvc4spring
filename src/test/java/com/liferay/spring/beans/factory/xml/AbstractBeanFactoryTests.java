 ...
 */
package com.liferay.spring.beans.factory.xml;
...
import com.liferay.spring.tests.sample.beans.MustBeInitialized;
import com.liferay.spring.tests.sample.beans.TestBean;
import com.liferay.spring.tests.sample.beans.factory.DummyFactory;
...
public abstract class AbstractBeanFactoryTests {
	...
	public void factorySingleton() throws Exception {
		...
		TestBean tb = (TestBean) getBeanFactory().getBean("singletonFactory");
		assertTrue("Singleton from factory has correct name, not " + tb.getName(),
			tb.getName().equals(DummyFactory.SINGLETON_NAME));
	...
	public void getInstanceByNonmatchingClass() {
		...
		try {
			...
			fail(
				"Rod bean is not of type BeanFactory; getBeanInstance(rod, BeanFactory.class) should throw BeanNotOfRequiredTypeException");
		}
		catch (BeanNotOfRequiredTypeException ex) {

			// So far, so good
			assertTrue("Exception has correct bean name", ex.getBeanName().equals("rod"));
			assertTrue("Exception requiredType must be BeanFactory.class",
	...
	/**
	 * Test that InitializingBean/BeanFactoryAware/DisposableBean objects receive the afterPropertiesSet() callback
	 * before BeanFactoryAware callbacks
	 */
	@Test
	...
	public void validEmpty() {
		...
		assertTrue("Valid empty has defaults",
			(ve.getName() == null) && (ve.getAge() == 0) && (ve.getSpouse() == null));
	}
	...
	public void xtestTypeMismatch() {
		...
		try {
			...
			assertTrue("We have rejected age in exception",
				ex.getPropertyAccessException("age").getPropertyChangeEvent().getNewValue().equals("34x"));
		}
	}
