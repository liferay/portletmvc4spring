...
/**
 * Provides utility methods that allow JUnit tests to {@link org.junit.Assume} certain conditions hold {@code true}. If
 * the assumption fails, it means the test should be skipped. Tests can be categorized into {@link TestGroup}s. Active
 * groups are enabled using the 'testGroups' system property, usually activated from the gradle command line:
 *
 * <pre>
   gradle test -PtestGroups="performance"
 * </pre>
 *
 * Groups can be specified as a comma separated list of values, or using the pseudo group 'all'. See {@link TestGroup}
...
public abstract class Assume {
	 ...
	 *
	 * <p>See <a href="https://java.net/jira/browse/MACOSX_PORT-355">MACOSX_PORT-355</a> issue.
	 *
	...
	/**
	 * Assume that a particular {@link TestGroup} has been specified before executing the supplied {@link Executable}.
	 *
	 * <p>If the assumption fails, the executable will not be executed, but no {@link AssumptionViolatedException} will
	 * be thrown.
	 *
	 * @param  group       the group that must be specified
