 ...
 *
 * <p><b>NOTE:</b> The WebRequestInterceptor is by default only applied to the Portlet <b>render</b> phase, which is
 * dealing with preparing and rendering a Portlet view. The Portlet action phase will only be intercepted with
 * WebRequestInterceptor calls if the {@code renderPhaseOnly} flag is explicitly set to {@code false}. In general, it is
 * recommended to use the Portlet-specific HandlerInterceptor mechanism for differentiating between action and render
 * interception.
 *
...
public class WebRequestHandlerInterceptorAdapter implements HandlerInterceptor {
	...
	/**
	 * Create a new WebRequestHandlerInterceptorAdapter for the given WebRequestInterceptor, applying to the render
	 * phase only.
	 *
	 ...
	 *
	 * @param  requestInterceptor  the WebRequestInterceptor to wrap
	 * @param  renderPhaseOnly     whether to apply to the render phase only ({@code true}) or to the action phase as
	 *                             well ({@code false})
	 */
