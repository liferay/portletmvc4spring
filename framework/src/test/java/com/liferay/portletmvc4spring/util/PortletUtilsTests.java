...
public final class PortletUtilsTests {
	...
	public void testClearAllRenderParameters() throws Exception {
		...
		assertEquals("The render parameters map is obviously not being cleared out.", 0,
			response.getRenderParameterMap().size());
	}

	@Test
	public void testClearAllRenderParametersDoesNotPropagateExceptionIfRedirectAlreadySentAtTimeOfCall()
		throws Exception {
		MockActionResponse response = new MockActionResponse() {
				@Override
		...
		PortletUtils.clearAllRenderParameters(response);
		assertEquals(
			"The render parameters map must not be cleared if ActionResponse.sendRedirect() has been called (already).",
	...
	public void testGetParametersStartingWith() throws Exception {
		...
		assertFalse(
			"Obviously not finding all of the correct parameters (is returning a parameter whose name does not start with the desired prefix",
	...
	public void testGetParametersStartingWithUnpicksScalarParameterValues() throws Exception {
		...
		assertEquals("Obviously not finding all of the correct parameters", 2, actualParameters.size());
		assertTrue("Obviously not finding all of the correct parameters", actualParameters.containsKey("William"));
		assertEquals("Not picking scalar parameter value out correctly", "Baskerville",
			actualParameters.get("William"));
		assertTrue("Obviously not finding all of the correct parameters", actualParameters.containsKey("Adso"));
		assertFalse(
			"Obviously not finding all of the correct parameters (is returning a parameter whose name does not start with the desired prefix",
			actualParameters.containsKey(badKey));
	...
	public void testGetParametersStartingWithYieldsEmptyNonNullMapWhenNoParamaterExistInRequest() throws Exception {
		...
		assertNotNull("PortletUtils.getParametersStartingWith(..) must never return a null Map", actualParameters);
		assertEquals("Obviously finding some parameters from somewhere (incorrectly)", request.getParameterMap().size(),
	...
	public void testGetParametersStartingWithYieldsEverythingIfTargetPrefixIsNull() throws Exception {
		...
		Map<?, ?> actualParameters = PortletUtils.getParametersStartingWith(request, null);
		assertNotNull("PortletUtils.getParametersStartingWith(..) must never return a null Map", actualParameters);
		assertEquals("Obviously not finding all of the correct parameters", request.getParameterMap().size(),
			actualParameters.size());
		assertTrue("Obviously not finding all of the correct parameters", actualParameters.containsKey("William"));
		assertTrue("Obviously not finding all of the correct parameters", actualParameters.containsKey("Adso"));
		assertTrue("Obviously not finding all of the correct parameters",
			actualParameters.containsKey("dominican_Bernard"));
	}
	...
	public void testGetRealPathInterpretsLocationAsRelativeToWebAppRootIfPathDoesNotBeginWithALeadingSlash()
		throws Exception {
	...
	public void testGetSessionAttributeDoes_Not_CreateANewSession() throws Exception {
		...
		assertNull("Must return null if session attribute does not exist (or if Session does not exist)",
			sessionAttribute);
	...
	public void testGetSessionMutexWithExistingSessionMutexReturnsTheExistingSessionMutex() throws Exception {
		...
		assertSame(
			"PortletUtils.getSessionMutex(..) must return the bound mutex attribute if a mutex has been bound as a Session attribute beforehand",
	...
	public void testGetSessionMutexWithNoExistingSessionMutexDefinedJustReturnsTheSessionArgument() throws Exception {
		...
		assertSame(
			"PortletUtils.getSessionMutex(..) must return the exact same PortletSession supplied as an argument if no mutex has been bound as a Session attribute beforehand",
	...
	@Test
	public void testPassAllParametersToRenderPhaseDoesNotPropagateExceptionIfRedirectAlreadySentAtTimeOfCall()
		throws Exception {
		MockActionRequest request = new MockActionRequest();
		request.setParameter("William", "Baskerville");
		request.setParameter("Adso", "Melk");
		...
		PortletUtils.passAllParametersToRenderPhase(request, response);
		assertEquals(
			"The render parameters map must not be being populated with the request parameters (Action.sendRedirect(..) aleady called).",
