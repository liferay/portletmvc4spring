...
import com.liferay.portletmvc4spring.HandlerExceptionResolver;
 ...
 *
 * <p>Provides a set of mapped handlers that the resolver should map to, and the {@link Ordered} implementation.
 *
...
public abstract class AbstractHandlerExceptionResolver implements HandlerExceptionResolver, Ordered {
	...
	/**
	 * Checks whether this resolver is supposed to apply (i.e. the handler matches in case of "mappedHandlers" having
	 * been specified), then delegates to the {@link #doResolveException} template method.
	 */
	@Override
	...
	@Override
	public ModelAndView resolveException(ResourceRequest request, ResourceResponse response, Object handler,
		Exception ex) {
	 ...
	 *
	 * <p>The exception mappings and the default error view will only apply to handlers of the specified types; the
	 * specified types may be interfaces or superclasses of handlers as well.
	 *
	 * <p>If no handlers or handler classes are set, the exception mappings and the default error view will apply to all
	 * handlers. This means that a specified default error view will be used as a fallback for all exceptions; any
	 * further HandlerExceptionResolvers in the chain will be ignored in this case.
	 */
	 ...
	 * Commons Logging, getting interpreted as a log category according to the logger's configuration.
	 *
	 ...
	 */
	protected abstract ModelAndView doResolveException(PortletRequest request, MimeResponse response, Object handler,
		Exception ex);
	 ...
	 *
	 * <p>The default implementation checks against the specified mapped handlers and handler classes, if any, and also
	 * checks the window state (according to the "renderWhenMinimize" property).
	 *
	 * @param   request  current portlet request
