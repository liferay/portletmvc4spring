...
/**
 * Implementation of the {@link com.liferay.portletmvc4spring.HandlerMapping} interface to map from the current
 ...
 *
 * <p>This is a combination of the methods used in {@link PortletModeHandlerMapping PortletModeHandlerMapping} and
 * {@link ParameterHandlerMapping ParameterHandlerMapping}. Unlike those two classes, this mapping cannot be initialized
 * with properties since it requires a two-level map.
 *
 * <p>The default name of the parameter is "action", but can be changed using {@link #setParameterName
 * setParameterName()}.
 *
 ...
 *
 * <pre class="code">
   &lt;bean id="portletModeParameterHandlerMapping" class="com.liferay.portletmvc4spring.handler.PortletModeParameterHandlerMapping"&gt;
   ...
   &lt;/bean&gt;</pre>
 *
 * <p>This mapping can be chained ahead of a {@link PortletModeHandlerMapping PortletModeHandlerMapping}, which can then
...
public class PortletModeParameterHandlerMapping extends AbstractMapBasedHandlerMapping<PortletModeParameterLookupKey> {
	 ...
	 *
	 * <p>Doing this is dangerous because the portlet mode can be changed by the portal itself and the only way to see
	 * that is a rerender of the portlet. If the same parameter value is legal in multiple modes, then a change in mode
	 * could result in a matched mapping that is not intended and the user could end up in a strange place in the
	 * application.
	 */
	...
	/**
	 * Set a Map with portlet mode names as keys and another Map as values. The sub-map has parameter names as keys and
	 * handler bean or bean names as values.
	 *
	...
	protected void registerHandler(PortletMode mode, String parameter, Object handler) throws BeansException,
		...
		if (!this.allowDuplicateParameters && this.parametersUsed.contains(parameter)) {
			throw new IllegalStateException("Duplicate entries for parameter [" + parameter +
				"] in different Portlet modes");
		}
