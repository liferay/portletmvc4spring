...
/**
 * Abstract base class for {@link com.liferay.portletmvc4spring.HandlerMapping} implementations that rely on a map which
 * caches handler objects per lookup key. Supports arbitrary lookup keys, and automatically resolves handler bean names
...
public abstract class AbstractMapBasedHandlerMapping<K> extends AbstractHandlerMapping {
	...
	/**
	 * Set whether to lazily initialize handlers. Only applicable to singleton handlers, as prototypes are always lazily
	 * initialized. Default is false, as eager initialization allows for more efficiency through referencing the handler
	 * objects directly.
	 *
	 * <p>If you want to allow your handlers to be lazily initialized, make them "lazy-init" and set this flag to true.
	 * Just making them "lazy-init" will not work, as they are initialized through the references from the handler
	...
	protected Object getHandlerInternal(PortletRequest request) throws Exception {
		...
		if (handler instanceof Map) {
			Map<PortletRequestMappingPredicate, Object> predicateMap = (Map<PortletRequestMappingPredicate, Object>)
				handler;
	 ...
	 * @param   handler    the handler instance or handler bean name String (a bean name will automatically be resolved
	 *                     into the corresponding handler bean)
	 *
	 ...
	 * @param   handler    the handler instance or handler bean name String (a bean name will automatically be resolved
	 *                     into the corresponding handler bean)
	 * @param   predicate  a predicate object for this handler (may be {@code null}), determining a match with the
	 *                     primary lookup key
	 *
	 * @throws  BeansException         if the handler couldn't be registered
	 * @throws  IllegalStateException  if there is a conflicting handler registered
