...
/**
 * Portlet-based implementation of the {@link org.springframework.web.context.request.RequestAttributes} interface.
 *
 * <p>Accesses objects from portlet request and portlet session scope, with a distinction between "session" (the
 * PortletSession's "portlet scope") and "global session" (the PortletSession's "application scope").
 *
...
public class PortletRequestAttributes extends AbstractRequestAttributes {
	...
	/**
	 * Constant identifying the {@link String} prefixed to the name of a destruction callback when it is stored in a
	 * {@link PortletSession}.
	 */
	public static final String DESTRUCTION_CALLBACK_NAME_PREFIX = PortletRequestAttributes.class.getName() +
		".DESTRUCTION_CALLBACK.";

	/**
	 * Constant that indicates global session scope.
	 *
	 * <p>This explicitly refers to a globally shared session, (a.k.a. PortletSession.APPLICATION_SCOPE) Else, it simply
	 * refers to the common session.
	 */
	...
	/**
	 * Update all accessed session attributes through {@code session.setAttribute} calls, explicitly indicating to the
