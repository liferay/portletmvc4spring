 ...
 *
 * <p>For usage with Portlets, this Servlet is necessary to force the portlet container to convert the PortletRequest to
 * a ServletRequest, which it has to do when including a resource via the PortletRequestDispatcher. This allows for
 * reuse of the entire Servlet-based View support even in a Portlet environment.
 *
 * <p>The actual mapping of the bridge servlet is configurable in the DispatcherPortlet, via a "viewRendererUrl"
 * property. The default is "/WEB-INF/servlet/view", which is just available for internal resource dispatching.
 *
...
public class ViewRendererServlet extends HttpServlet {
	...
	/**
	 * Request attribute to hold current web application context. Otherwise only the global web app context is
	...
	/**
	 * Process this request, handling exceptions. The actually event handling is performed by the abstract {@code
