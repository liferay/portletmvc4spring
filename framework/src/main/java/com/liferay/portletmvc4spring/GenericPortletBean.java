...
import com.liferay.portletmvc4spring.context.PortletContextResourceLoader;
 ...
 *
 * <p>This portlet leaves request handling to subclasses, inheriting the default behaviour of GenericPortlet ({@code
 * doDispatch}, {@code processAction}, etc).
 *
 * <p>This portlet superclass has no dependency on a Spring application context, in contrast to the FrameworkPortlet
 * class which loads its own context.
 *
...
public abstract class GenericPortletBean extends GenericPortlet implements EnvironmentCapable, EnvironmentAware {
	...
	/**
	 * Overridden method that simply returns {@code null} when no PortletConfig set yet.
	 *
	 ...
	 *
	 * @throws  PortletException  if bean properties are invalid (or required properties are missing), or if subclass
	 ...
	 * {@inheritDoc}
	 *
	 * @throws  IllegalArgumentException  if environment is not assignable to {@code ConfigurableEnvironment}.
	 */
	@Override
	...
	/**
	 * Subclasses can invoke this method to specify that this property (which must match a JavaBean property they
	 * expose) is mandatory, and must be supplied as a config parameter. This method would normally be called from a
	...
	/**
	 * Create and return a new {@link StandardPortletEnvironment}. Subclasses may override in order to configure the
	 * environment or specialize the environment type returned.
	 */
	...
	/**
	 * Subclasses may override this to perform custom initialization. All bean properties of this portlet will have been
	 * set before this method is invoked. This default implementation does nothing.
	 *
