 ...
 *
 * <p>Interface that must be implemented for each handler type to handle a request. This interface is used to allow the
 * DispatcherPortlet to be indefinitely extensible. The DispatcherPortlet accesses all installed handlers through this
 * interface, meaning that it does not contain code specific to any handler type.
 *
 * <p>Note that a handler can be of type Object. This is to enable handlers from other frameworks to be integrated with
 * this framework without custom coding.
 *
 * <p>This interface is not intended for application developers. It is available to handlers who want to develop their
 * own web workflow.
 *
 * <p>Note: Implementations can implement the Ordered interface to be able to specify a sorting order and thus a
 * priority for getting applied by DispatcherPortlet. Non-Ordered instances get treated as lowest priority.
 *
 * @author  John A. Lewis
 * @since   2.0
 * @see     com.liferay.portletmvc4spring.mvc.SimpleControllerHandlerAdapter
 */
public interface HandlerAdapter {
	 ...
	 * @param   handler   handler to use. This object must have previously been passed to the {@code supports} method of
	...
	/**
	 * Given a handler instance, return whether or not this HandlerAdapter can support it. Typical HandlerAdapters will
	 * base the decision on the handler type. HandlerAdapters will usually only support one handler type each.
	 *
