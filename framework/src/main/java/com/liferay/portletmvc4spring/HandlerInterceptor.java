...
/**
 * Workflow interface that allows for customized handler execution chains. Applications can register any number of
 * existing or custom interceptors for certain groups of handlers, to add common pre-processing behavior without needing
 * to modify each handler implementation.
 *
 * <p>A {@code HandlerInterceptor} gets called before the appropriate {@link HandlerAdapter} triggers the execution of
 ...
 *
 * <p>Typically an interceptor chain is defined per {@link HandlerMapping} bean, sharing its granularity. To be able to
 * apply a certain interceptor chain to a group of handlers, one needs to map the desired handlers via one {@code
 * HandlerMapping} bean. The interceptors themselves are defined as beans in the application context, referenced by the
 ...
 *
 * <p>As a basic guideline, fine-grained handler-related pre-processing tasks are candidates for {@code
 * HandlerInterceptor} implementations, especially factored-out common handler code and authorization checks. On the
 * other hand, a {@code Filter} is well-suited for request content and view content handling, like multipart forms and
 * GZIP compression. This typically shows when one needs to map the filter to certain content types (e.g. images), or to
 * all requests.
 *
 * <p>Be aware that filters cannot be applied to portlet requests (they only operate on servlet requests), so for
 * portlet requests interceptors are essential.
 *
 * <p>If we assume a "sunny day" request cycle (i.e. a request where nothing goes wrong and all is well), the workflow
 * of a {@code HandlerInterceptor} will be as follows:
 *
 * <p><b>Action Request:</b>
 *
 * <p>
 * <ol>
 ...
 * @see     HandlerMapping
 * @see     com.liferay.portletmvc4spring.handler.AbstractHandlerMapping#setInterceptors
...
public interface HandlerInterceptor {
	 ...
	 * @param   ex        exception thrown on handler execution, if any (only included as additional context information
	 ...
	 *
	 * <p>Called after a {@link HandlerAdapter} actually invoked the handler, but before the {@code DispatcherPortlet}
	 * renders the view. Can thus expose additional model objects to the view via the given {@link ModelAndView}.
	 *
	 * <p>{@code DispatcherPortlet} processes a handler in an execution chain, consisting of any number of interceptors,
	 ...
	 * @throws  Exception  in case of errors
	 */
	void postHandleResource(ResourceRequest request, ResourceResponse response, Object handler,
		ModelAndView modelAndView) throws Exception;

	/**
	 * Intercept the execution of a handler in the action phase.
	 *
	 * <p>Called after a HandlerMapping determines an appropriate handler object to handle an {@link ActionRequest}, but
	 * before said HandlerAdapter actually invokes the handler.
	 *
	 * <p>{@link DispatcherPortlet} processes a handler in an execution chain, consisting of any number of interceptors,
	 ...
	 *          Else, {@code DispatcherPortlet} assumes that this interceptor has already dealt with the response itself
	 *
	 * @throws  Exception  in case of errors
	 */
	 ...
	 * @throws  Exception  in case of errors
	 */
	boolean preHandleEvent(EventRequest request, EventResponse response, Object handler) throws Exception;

	/**
	 * Intercept the execution of a handler in the render phase.
	 *
	 * <p>Called after a HandlerMapping determines an appropriate handler object to handle a {@link RenderRequest}, but
	 * before said HandlerAdapter actually invokes the handler.
	 *
	 * <p>{@link DispatcherPortlet} processes a handler in an execution chain, consisting of any number of interceptors,
	 * with the handler itself at the end. With this method, each interceptor can decide to abort the execution chain,
	 ...
	 * @throws  Exception  in case of errors
	 */
	boolean preHandleRender(RenderRequest request, RenderResponse response, Object handler) throws Exception;
	 ...
	 * @throws  Exception  in case of errors
	 */
	boolean preHandleResource(ResourceRequest request, ResourceResponse response, Object handler) throws Exception;
