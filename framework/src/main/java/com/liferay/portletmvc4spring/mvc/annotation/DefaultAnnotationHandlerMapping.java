...
import com.liferay.portletmvc4spring.handler.AbstractMapBasedHandlerMapping;
import com.liferay.portletmvc4spring.handler.PortletRequestMethodNotSupportedException;


/**
 * Implementation of the {@link com.liferay.portletmvc4spring.HandlerMapping} interface that maps handlers based on
 * portlet modes expressed through the {@link RequestMapping} annotation at the type or method level.
 *
 * <p>Registered by default in {@link com.liferay.portletmvc4spring.DispatcherPortlet}. <b>NOTE:</b> If you define
 ...
 * the {@link com.liferay.portletmvc4spring.mvc.Controller} interface). However, {@link Controller} is required for
 * detecting {@link RequestMapping} annotations at the method level.
 *
 * <p><b>NOTE:</b> Method-level mappings are only allowed to narrow the mapping expressed at the class level (if any). A
 * portlet mode in combination with specific parameter conditions needs to uniquely map onto one specific handler bean,
 * not spread across multiple handler beans. It is strongly recommended to co-locate related handler methods into the
 * same bean.
 *
 ...
 * com.liferay.portletmvc4spring.mvc.SimpleControllerHandlerAdapter} apply.
 *
...
public class DefaultAnnotationHandlerMapping extends AbstractMapBasedHandlerMapping<PortletMode> {
	...
	/**
	 * Calls the {@code registerHandlers} method in addition to the superclass's initialization.
	 *
	 * @see  #detectHandlers
	 */
	@Override
	 ...
	 *
	 * @return  {@code true} if at least 1 handler method has been registered; {@code false} otherwise
	 */
	protected boolean detectHandlerMethods(Class<?> handlerType, final String beanName,
		...
		for (String beanName : beanNames) {
			...
			if (mapping != null) {
				...
				if (registerHandlerType) {
					AbstractParameterMappingPredicate predicate = new TypeLevelMappingPredicate(params,
							mapping.headers(), mapping.method());
	...
	private abstract static class AbstractParameterMappingPredicate implements PortletRequestMappingPredicate {
	...
	private static class ActionMappingPredicate extends AbstractParameterMappingPredicate
		...
		public boolean match(PortletRequest request) {
			return (PortletRequest.ACTION_PHASE.equals(request.getAttribute(PortletRequest.LIFECYCLE_PHASE)) &&
					("".equals(this.actionName) ||
						this.actionName.equals(request.getParameter(ActionRequest.ACTION_NAME))) &&
					super.match(request));
		}
	...
	private static class RenderMappingPredicate extends AbstractParameterMappingPredicate
		implements SpecialRequestTypePredicate {
	...
	private static class ResourceMappingPredicate implements PortletRequestMappingPredicate,
		...
		public boolean match(PortletRequest request) {
			return (PortletRequest.RESOURCE_PHASE.equals(request.getAttribute(PortletRequest.LIFECYCLE_PHASE)) &&
					("".equals(this.resourceId) || this.resourceId.equals(
							((ResourceRequest) request).getResourceID())));
		}
		...
		public void validate(PortletRequest request) throws PortletException {
			...
			if (!PortletAnnotationMappingUtils.checkHeaders(this.headers, request)) {
				throw new PortletRequestBindingException("Header conditions \"" +
					StringUtils.arrayToDelimitedString(this.headers, ", ") + "\" not met for actual request");
			}
			...
			if (!this.methods.isEmpty()) {
				...
				if (!this.methods.contains(method)) {
					throw new PortletRequestMethodNotSupportedException(method,
						StringUtils.toStringArray(this.methods));
				}
			}
		}
	}
