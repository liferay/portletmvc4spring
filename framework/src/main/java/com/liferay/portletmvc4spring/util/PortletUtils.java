...
public abstract class PortletUtils {
	...
	/**
	 * Expose the given Map as request attributes, using the keys as attribute names and the values as corresponding
	 * attribute values. Keys must be Strings.
	 *
	...
	/**
	 * Return an appropriate response object of the specified type, if available, unwrapping the given response as far
	 * as necessary.
	 *
	...
	/**
	 * Get the specified session attribute in the given scope, creating and setting a new attribute if no existing
	 * found. The given class needs to have a public no-arg constructor. Useful for on-demand state objects in a web
	 * tier, like shopping carts.
	 *
	 * @param   session  current portlet session
	 * @param   name     the name of the session attribute
	 * @param   clazz    the class to instantiate for a new attribute
	...
	public static String getRealPath(PortletContext portletContext, String path) throws FileNotFoundException {
		...
		if (realPath == null) {
			throw new FileNotFoundException("PortletContext resource [" + path +
	...
	/**
	 * Check the given request for a session attribute of the given name in the given scope. Throws an exception if
	 * there is no session or if the session has no such attribute in that scope.
	 *
	 * <p>Does not create a new session if none has existed before!
	 *
	 * @param   request  current portlet request
	...
	/**
	 * Check the given request for a session attribute of the given name in the given scope. Returns {@code null} if
	 * there is no session or if the session has no such attribute in that scope. Does not create a new session if none
	 * has existed before!
	 *
	 * @param   request  current portlet request
	 * @param   name     the name of the session attribute
	...
	/**
	 * Return the best available mutex for the given session: that is, an object to synchronize on for the given
	 * session.
	 *
	...
	/**
	 * Return the full name of a specific input type="submit" parameter if it was sent in the request, either via a
	 * button (directly with name) or via an image (name + ".x" or name + ".y").
	 *
	 * @param   request  current portlet request
	 ...
	 *
	 * @param   request      current portlet request
	 * @param   paramPrefix  the parameter prefix to check for (e.g. "_target" for parameters like "_target1" or
	 *                       "_target2")
	...
	/**
	 * Check if a specific input type="submit" parameter was sent in the request, either via a button (directly with
	 * name) or via an image (name + ".x" or name + ".y").
	 *
	 * @param   request  current portlet request
	 * @param   name     name of the parameter
	 *
	...
	/**
	 * Serve the resource as specified in the given request to the given response, using the PortletContext's request
	 * dispatcher.
	 *
	...
	/**
	 * Set the session attribute with the given name to the given value in the given scope. Removes the session
	 * attribute if value is {@code null}, if a session existed at all. Does not create a new session if not necessary!
	 *
	 * @param  request  current portlet request
	 * @param  name     the name of the session attribute
	 * @param  value    the value of the session attribute
